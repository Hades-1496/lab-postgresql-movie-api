...
const actualizarPelicula = async (req, res, next) => {
  try {
    const id = Number(req.params.id);
  const { titulo, director_id, anio, genero_id, nota } = req.body;
  const actualizada = await db.actualizar(id, { titulo, director_id, anio: Number(anio), genero_id, nota: nota? Number(nota) : null});
...
const eliminarPelicula = async (req, res, next) => { //Casi idéntica a búsqueda de películas por id.
  try {
    const id = Number(req.params.id);
...
const crearResenias = async (req, res, next) => {
    ...
    const nueva = await db.crearResena(peliculaId, {autor, texto, puntuacion: Number(puntuacion)});
